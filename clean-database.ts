...
async function checkAndDelete() {
  try {
    ...
    imgResult.rows.forEach((row: any) => console.log(`  ${row.category}: ${row.count}`));

    const prodResult = await query('SELECT category, COUNT(*) FROM products GROUP BY category ORDER BY category');
    console.log('Products table:');
    prodResult.rows.forEach((row: any) => console.log(`  ${row.category}: ${row.count}`));
    ...
    console.log('Images table:');
    remainingImg.rows.forEach((row: any) => console.log(`  ${row.category}: ${row.count}`));

    const remainingProd = await query('SELECT category, COUNT(*) FROM products GROUP BY category ORDER BY category');
    console.log('Products table:');
    remainingProd.rows.forEach((row: any) => console.log(`  ${row.category}: ${row.count}`));
